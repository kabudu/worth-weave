...
export function AiSettingsPanel({ runtime, model }: { runtime: string | null; model: string | null }) {
  ...
  return <section className="ai-settings">
    ...
    <div><p>{recommendation.data ? `Best option for this Mac: ${recommendation.data.runtime_name} with ${recommendation.data.model}.` : "Checking this Mac…"}</p><button className="secondary-button" type="button" disabled={!recommendation.data || setup.isPending} onClick={() => setup.mutate()}>{setup.isPending ? "Setting up…" : runtime ? "Set up again" : "Set up private AI"}</button>{setup.isError && <small className="form-error">{String(setup.error)}</small>}</div>
  </section>;
}
