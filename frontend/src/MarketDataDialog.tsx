...
export function MarketDataDialog({ open, onClose, holdings, currencies, reportingCurrency }: Props) {
  ...
  const instrumentSelect = <label>Instrument<select value={effectiveInstrument} onChange={(event) => selectInstrument(event.target.value)}>{holdings.map((holding) => <option key={`${holding.account_id}-${holding.instrument_id}`} value={holding.instrument_id}>{holding.symbol ?? holding.instrument_id} · {holding.account_name}</option>)}</select></label>;
  return <dialog ref={dialogRef} className="market-dialog" onClose={onClose}>
    <div className="dialog-topline"><div><span className="section-kicker">Manual market data</span><h2>Prices, exchange rates &amp; categories</h2></div><button type="button" className="dialog-close" onClick={onClose} aria-label="Close market data">×</button></div>
