...
export function PortfolioView({ holdings, valuation, attribution, allocation, snapshots, currencies, reportingCurrency, reconciliation }: PortfolioProps) {
  ...
  return <section className="report-page">
    <header className="report-title-row"><div><span className="section-kicker">Based on your imported history</span><h1>Your investments</h1><p>Worthweave uses your account history to work out how much you own and what you paid.</p></div><div className="valuation-total"><span>Portfolio value · {reportingCurrency}</span><strong>{money(valuation?.total_value ?? null, reportingCurrency)}</strong>{valuation && !valuation.total_value && <small>{valuation.missing_price_count} prices · {valuation.missing_fx_count} exchange rates missing</small>}<button type="button" onClick={() => setMarketOpen(true)}>Update prices and rates</button></div></header>
    {attribution && <AttributionPanel report={attribution} />}
    {holdings.length === 0 ? <EmptyState title="No investments to show yet" copy="Import your account history to see what you own." /> : <><div className="report-table-wrap"><table><thead><tr><th>Investment</th><th>Account</th><th>Quantity</th><th>Average cost</th><th>Amount invested</th><th>Current value</th><th>Value in {reportingCurrency}</th><th>Gain / loss</th></tr></thead><tbody>{holdings.map((holding) => {
...
function ReconciliationPanel({ items }: { items: ReconciliationItem[] }) {
  ...
  return <section className="reconciliation-card" aria-labelledby="reconciliation-title"><div><span className="section-kicker">Holdings check</span><h2 id="reconciliation-title">Do your holdings match?</h2><p>{matched} of {items.length} investments match the latest quantities in your broker file.</p></div><div className="reconciliation-list">{items.map((item) => <div key={`${item.account_id}-${item.instrument_id}`}><span className={`reconciliation-status ${item.status}`}>{item.status === "matched" ? "Matches" : item.status === "unavailable" ? "Not in broker file" : "Needs checking"}</span><strong>{item.instrument_id}</strong><small>{item.account_name} · Worthweave {item.ledger_quantity} · broker file {item.broker_quantity ?? "not included"}{item.as_of ? ` · ${item.as_of}` : ""}</small></div>)}</div></section>;
}
