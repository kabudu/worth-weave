...
function AttributionPanel({ report }: { report: TotalReturnAttribution }) {
  ...
  return <section className="attribution-card" aria-labelledby="attribution-title">
    <div className="attribution-heading"><div><span className="section-kicker">Investment return</span><h2 id="attribution-title">What changed your return</h2><p>{report.coverage_start && report.coverage_end ? `${report.coverage_start} to ${report.coverage_end}` : "Import history to get started"} · {report.reporting_currency}</p></div><div className={`attribution-total ${report.status}`}><span>{report.total_return ? "Total return" : report.status === "partial" ? "Partial return" : "Not calculated"}</span><strong>{money(headline, report.reporting_currency)}</strong><small>{report.status === "complete" ? "Complete for the imported dates" : report.status === "partial" ? "Available records only—not a complete return" : "Not enough information yet"}</small></div></div>
