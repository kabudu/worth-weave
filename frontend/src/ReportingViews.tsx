...
export function PortfolioView({ holdings, valuation, attribution, allocation, snapshots, currencies, reportingCurrency, reconciliation }: PortfolioProps) {
  ...
  return <section className="report-page">
    <header className="report-title-row"><div><span className="section-kicker">Based on your imported history</span><h1>Your investments</h1><p>Current broker positions set quantities when they are available. Imported transactions provide cost and return history.</p></div><div className="valuation-total"><span>Portfolio value · {reportingCurrency}</span><strong>{money(valuation?.total_value ?? null, reportingCurrency)}</strong>{valuation && !valuation.total_value && <small>{valuation.missing_price_count} current prices · {valuation.missing_fx_count} exchange rates still needed</small>}<button type="button" onClick={() => setMarketOpen(true)}>Add missing prices and rates</button></div></header>
...
function ReconciliationPanel({ items }: { items: ReconciliationItem[] }) {
  ...
  return <section className="reconciliation-card" aria-labelledby="reconciliation-title"><div><span className="section-kicker">History check</span><h2 id="reconciliation-title">Does your history explain today’s positions?</h2><p>{matched} of {items.length} current broker positions are fully explained by imported trades.{incomplete > 0 ? ` ${incomplete} need earlier history or investment identifiers.` : ""}</p></div><div className="reconciliation-list">{items.map((item) => <div key={`${item.account_id}-${item.instrument_id}`}><span className={`reconciliation-status ${item.status}`}>{item.status === "matched" ? "Explained" : item.status === "unavailable" ? "History incomplete" : "Quantity differs"}</span><strong>{item.instrument_id}</strong><small>{item.account_name} · transaction history {item.ledger_quantity} · current broker position {item.broker_quantity ?? "not included"}{item.as_of ? ` · ${item.as_of}` : ""}</small></div>)}</div></section>;
}
